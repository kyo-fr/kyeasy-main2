...
public class RedisCaches {
    ...
    public <T> T getCacheObject(final String key, Class<T> clazz)
    {
        ...
        if (clazz.isInstance(value)) {
            return clazz.cast(value); // 安全转换
        }
        return null; // 或者根据需求返回默认值
    }
    ...
    {
        Long delete = redisTemplate.delete((Collection<String>) collection);
    ...
    public <T> List<T> getCacheList(final String key, Class<T> clazz)
    {
        ...
        return rawList.stream()
                .filter(clazz::isInstance) // 过滤出指定类型的元素
                .map(clazz::cast)          // 安全转换
    ...
    public <T> BoundSetOperations<String, Object> setCacheSet(final String key, final Set<T> dataSet)
    {
        ...
        for (T data : dataSet) {
            setOperation.add(data); // 添加数据到 Set
        }
        return setOperation;
    }
    ...
    public <T> Set<T> getCacheSet(final String key, Class<T> clazz)
    {
        ...
        return members.stream().filter(clazz::isInstance).map(clazz::cast).collect(Collectors.toSet());
    }
    ...
    public <T> List<T> getMultiCacheMapValue(final String key, final Collection<Object> hKeys)
    {
        ...
        List<Object> rawValues = hashOperations.multiGet(key, hKeys);

        // 安全地转换为目标类型
        return rawValues.stream()
                .filter(Objects::nonNull) // 过滤掉 null 值
                .map(value -> (T) value) // 强制转换
                .collect(Collectors.toList());
    }
