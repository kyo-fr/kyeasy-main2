...
import org.ares.cloud.config.query.AbstractQuery;
...
public class GenUtils {
     ...
     */
    public static List<ModelEntity> getTableList(GenDataSource datasource) {
        List<ModelEntity> tableList = new ArrayList<>();
        try {
            ...
            while (rs.next()) {
                ModelEntity table = new ModelEntity();
     ...
     * @param tableName  表名
     */
    public static ModelEntity getTable(GenDataSource datasource, String tableName) {
        try {
            AbstractQuery query = datasource.getDbQuery();
            String sql = query.tableSql(tableName);
            // 查询数据
     ...
     * @param tableName  表名
     */
    public static List<ModelFieldEntity> getTableFieldList(GenDataSource datasource, String modelId, String tableName) {
        List<ModelFieldEntity> tableFieldList = new ArrayList<>();
        ...
        try {
            ...
            try (PreparedStatement preparedStatement = datasource.getConnection().prepareStatement(tableFieldsSql);
                 ResultSet rs = preparedStatement.executeQuery()) {

                while (rs.next()) {
                    ModelFieldEntity field = new ModelFieldEntity();
